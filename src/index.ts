...
class KnowledgeGraphManager {
  ...
  getChildren(id: string, type: string) {
    return this.relations.filter((r) => r.from === id && r.type === type).map((r) => this.entities[r.to]).filter(Boolean) as DagNode[];
  }
...
class ActorCriticEngine {
  ...
  async actorThink(input: { thought: string; needsMore: boolean; branchLabel?: string; tags?: string[]; artifacts?: ArtifactRef[] }): Promise<DagNode> {
    ...
    if (artifacts && artifacts.length) {
      for (const art of artifacts) {
        const artEntity: ArtifactRef = { ...art, id: art.id ?? uuid() };
  ...
  exportPlan(filterTag?: string): unknown {
    ...
    return nodes.map((n) => ({ id: n.id, thought: n.thought, tags: n.tags, branchLabel: n.branchLabel, verdict: n.verdict, parents: n.parents, artifacts: n.artifacts?.map((a) => ({ name: a.name, uri: a.uri })) }));
  }
...
async function main() {
  ...
  const ActorThinkSchema = {
    ...
    })).optional().describe("Supporting files / links."),
    revisesNodeId: z.string().optional().describe("If this is a self‑initiated revision, node id being revised."),
  }

  server.tool("actor_think",
    ActorThinkSchema,
    async (args) => ({
